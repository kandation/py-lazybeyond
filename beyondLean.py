...
class BeyondLazyApi:
    def __init__(self, session: requests.Session):
        ...
        self.__course_info = {}
        self.__curr_uri = ''
        self.__view_course_uri = ''

    def check_cookie(self):
        show('_____Check_Cookies_____')
    ...
    def get_task_by_index(self, index):
        ...
        show(f'===== Get task [{index}] ======')
        print('Get', self.__curr_uri)
    ...
    def take(self, id, ref_uri):
        show('===== Take =====')
        print('id:: ', id, 'RefId::', ref_uri)
        ...
        headers = {
            ...
            'DNT': '1',
            'Origin': 'https://vcubelms.com',
            'Referer': f'https://vcubelms.com/beyondtrainingplus/Course/ViewCourse.aspx?p={ref_uri}',
            'Sec-Fetch-Dest': 'empty',
            'Sec-Fetch-Mode': 'cors',
            'Sec-Fetch-Site': 'same-origin',
            'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/111.0.0.0 Safari/537.36 Edg/111.0.1661.41',
        ...
        self.__course_info = response.json()
    ...
    def get_start(self):
        ...
        response = self.session.request("GET", url, headers=headers, data=payload, verify=False, allow_redirects=False)
        new_location = response.headers.get('Location')

        self.__view_course_uri = new_location
        return new_location

    def get_view_course(self):
        show('===== View Course =====')
        if not self.__view_course_uri:
            raise Exception("Not select Course")

        url = f"https://vcubelms.com/{self.__view_course_uri}"
        print(url)
...
class SessionManager:
    ...
    def load(self):
        show('_____Load_Session_____')
        with open('my/cookie.json', 'rb') as f:
            # self.__session.cookies.update(pickle.load(f))
            self.__session.cookies.update(json.loads(f.read()))
